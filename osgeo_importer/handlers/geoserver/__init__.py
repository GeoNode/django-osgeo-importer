...
def configure_time(resource,
    ...
    """
    time_info = DimensionInfo(name, enabled, presentation, resolution, units,
                              unitSymbol, **kwargs)
...
class GeoServerTimeHandler(GetModifiedFieldsMixin, GeoserverHandlerMixin):
    ...
    def can_run(self, layer, layer_config, *args, **kwargs):
        ...
        if not any([
                layer_config.get('start_date', None),
                layer_config.get('end_date', None)
        ]):
            return False
...
class GeoserverPublishHandler(GeoserverHandlerMixin):
    ...
    def multiprocess_safe_create_store(catalog, conn_str, workspace_name):
        ...
        try:
            store = catalog.create_datastore(
                conn_str['name'], workspace=workspace_name)
    ...
    def get_or_create_datastore(self, layer_config, request_user):
        ...
        ensure_workspace_exists(self.catalog, self.workspace,
                                self.workspace_namespace_uri)

        if use_conn_str is None:
            raise Exception(
                'No connection string available to create datastore')
        ...
        try:
            ...
            if connection_string is not None:
                if connection_string['type'] == 'geogig':
                    ...
                    init_response = init_geogig_repo(payload,
            ...
            s = self.multiprocess_safe_create_store(self.catalog, use_conn_str,
                                                    self.workspace)
        ...
        if (s.type is None and use_conn_str.get('type') == 'geogig'):
            ...
            try:
                ...
                s = self.multiprocess_safe_create_store(
                    self.catalog, use_conn_str, self.workspace)
    ...
    def geogig_handler(self, store, layer, layer_config, request_user):
        ...
        repo = store.name
        repo_url = self.catalog.service_url.replace(
            '/rest', '/geogig/repos/{0}/'.format(repo))
        ...
        logger.debug("""response status_code {} \n
                        response headers {} \n
                        request headers {} \n
                     """.format(transaction.status_code, transaction.headers,
        ...
        import_command = requests.get(
            repo_url + 'postgis/import.json', params=params, **request_params)
    ...
    def handle(self, layer, layer_config, *args, **kwargs):
        ...
        return self.catalog.publish_featuretype(
            layer, self.get_or_create_datastore(layer_config, request_user),
        ...
        try:
            ...
            for dep in resp.json()['about']['resource']:
                if 'geogig-api' in dep['@name']:
                    ...
                    break
        except (TypeError, KeyError, requests.exceptions.ReadTimeout,
...
class GeoserverPublishCoverageHandler(GeoserverHandlerMixin):
    ...
    def handle(self, layer, layer_config, *args, **kwargs):
        ...
        ensure_workspace_exists(self.catalog, self.workspace_name,
                                self.workspace_namespace_uri)
        ...
        """
        UPLOAD_RASTER = getattr(
            settings, 'OSGEO_IMPORTER_UPLOAD_RASTER_TO_GEOSERVER', True)
        if UPLOAD_RASTER:
            return self.catalog._create_coveragestore(name, layer, workspace,
                                                      False, False)
        else:
            return self.catalog._create_coveragestore(
                name, "file:{}".format(layer), workspace, False, True)
...
class GeoWebCacheHandler(GeoserverHandlerMixin):
    ...
    def gwc_url(self, layer):
        ...
        return self.catalog.service_url.replace(
            'rest', 'gwc/rest/layers/{workspace}:{layer_name}.xml'.format(
    ...
    def handle(self, layer, layer_config, *args, **kwargs):
        ...
        return self.catalog.http.request(
            self.gwc_url(self.layer),
            method="POST",
            body=self.config(
                regex_parameter_filter=regex_filter, name=self.layer.name))
...
class GenericSLDHandler(GeoserverHandlerMixin):
    ...
    def can_run(self, layer, layer_config, *args, **kwargs):
        ...
        if self.layer and self.layer.default_style:
            return self.layer.default_style.name in [
                'generic', 'polygon', 'point', 'line', 'raster'
            ]
        else:
            return False
...
class GeoServerStyleHandler(GeoserverHandlerMixin):
    ...
    def can_run(self, layer, layer_config, *args, **kwargs):
        ...
        """
        if not any([
                layer_config.get('default_style', None),
                layer_config.get('styles', None)
        ]):
            return False
    ...
    def handle(self, layer, layer_config, *args, **kwargs):
        ...
        path = os.path.join(UPLOAD_DIR,
                            str(self.importer.upload_file.upload.id))
        ...
        for sld in all_slds:
            with open(os.path.join(path, sld)) as s:
                ...
                style = self.catalog.get_style(
                    sldname, workspace=self.workspace)
                if sld == default_sld:
                    default_style = style
                styles.append(style)
